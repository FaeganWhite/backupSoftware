...
# Makes proxies for all the video files in the given location
def makeProxies(location):
    # If the proxies directory hasn't already been created
    if not os.path.isdir(os.path.join(location,"Proxies")):
        # Make the project directory
        os.mkdir(os.path.join(location,"Proxies"))
    # Create a list to store media files to be converted
    media_files = []
    # Create a list of media files which have already been converted
    converted_media_files = []
    # Get all the media types
    media_types = getMediaTypes()
    # For each media type
    for media_type in media_types:
        # Add any video files to the list of media files
        media_files += folderSearch(media_type, location, False, False, 'files')
        # Add any converted media files tot he list of converted media files
        converted_media_files += folderSearch(media_type, os.path.join(location,"Proxies"), False, False, 'files')
    # For each file 
    for file in media_files:
        # Set the conversion to true
        needs_converting = True
        # For each converted file
        for file2 in converted_media_files:
            # If the names match
            if (file.split('.')[0] in file2.split('.')[0]):
                # Set converting to false as it already exists
                needs_converting = False
        # If it needs converting
        if needs_converting:
            print("Making proxy", file)
            # Make a proxy of the video file
            makeProxy(file, os.path.join(location,"Proxies"))

# Function using ffmpeg to create a proxy of a video
def makeProxy(input_path, output_path):
    # Get the name of the original file
    file_name = input_path[input_path.rfind('/')+1:]
    # Add proxy to the name
    proxy_name = file_name.split('.')[0]+"_Proxy."+file_name.split('.')[1]
    # Convert the video at the given path to a proxy and output it to the given location
        ...
        if (free > footage_size*1.1):
            ...
            if verifyCopy(drive_path, new_path):
                ...
                if (proxy == True):
                    ...
                    # Make proxies at the new location
